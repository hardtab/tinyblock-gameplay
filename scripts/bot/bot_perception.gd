...
static func build(snapshot: Dictionary, own_player_id: String, radius: float = DEFAULT_RADIUS, now_msec: int = 0) -> Dictionary:
	...
	for key in ["self_defense", "social_emoji", "social_target_id", "preferred_player_distance", "creature_attack_distance", "bow_attack_distance", "pvp_world", "duel_started", "enemy_player_id", "recipes", "achievements", "equipment_slots", "craft_pending_output", "craft_retry_after_msec", "craft_blocked_outputs"]:
		if snapshot.has(key):
